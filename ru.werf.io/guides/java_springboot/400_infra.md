...
Обычно для каждого стенда (production, staging, test и т.п.) и приложения используется отдельное пространство имён. Таким образом, для приложения `werf-guided-spring` будут созданы пространства имён `werf-guided-spring-production`, `werf-guided-spring-staging` и т.п.
...
- `<namespace>` — название пространства имён в Kubernetes (например, `werf-guided-spring-production`);
...
В каждом репозитории с кодом, с которым вы будете работать, вам надо будет **прописать в GitLab специальную переменную**. Зайдите в репозиторий и в левой панели нажмите `Settings` -> `CI/CD`. В этом разделе, в главе `Variables`, нужно прописать переменную окружения `KUBECONFIG_BASE64` с нашим `.kube/config`, зашифрованным base64.
