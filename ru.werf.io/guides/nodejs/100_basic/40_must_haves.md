...
- Во-первых, иметь общее представление о принципах и механизмах работы werf.
...
- Рано или поздно, как следствие доработок приложения и регулярных релизов, вы можете столкнуться с проблемой **места в хранилище образов** и необходимостью делать очистку системы.

Все эти вопросы подробнее рассмотрены далее.

## Основные принципы и механизмы работы werf

Как уже говорилось ранее, werf - это инструмент, позволяющий объединить воедино такие вещи как Git, Docker, Helm и Kubernetes с любой CI/CD системой. При этом пользователь максимально избавлен от необходимости вникать в детали - большая часть логики реализована под капотом werf, пользователю же остается лишь оперировать не образами и контейнерами, а приложением в целом, а, при необходимости, его компонентами. При этом в werf реализован подход при котором Git используется как единый источник истины - «гитерминизм».

## Основные термины, понятия и сущности

Гитерминизм - все, что касается приложения и его состояния в кластере должно храниться в репозитории и только там, в случае если приложения будет изменено напрямую внутри кластера - werf это обнаружит и автоматически откатит к состоянию, определенному в Git-репозитории.

## Необходимые команды, которые должен знать каждый

Основные команды, которые вам необходимо знать на всех этапах сборки и деплоя приложения представлены ниже:
`werf build` - сборка образа и его деплой в кластер.
`werf render` - проводит весь цикл сборки и генерации чартов и, вместо деплоя полученного релиза в Kubernetes, выводит полученные чарты.
`werf helm list -A` - отобразить все установленные релизы.
`werf cleanup` - безопасно удаляет образы, которые более не требуются.
...
Чтобы **гарантировать** воспроизводимость, werf следует принципам гитерминизма, детерминируя работу с проектом helm list -A
...
Режим разработчика позволяет не делать промежуточные коммиты а работать с состоянием stage.
Режим активируется опцией `--dev` или соответствующей переменной окружения `WERF_DEV`. В этом режиме поддерживается два режима работы, которые регулируются опцией `--dev-mode` (по умолчанию _simple_):
...
_Обратите внимание, что, как и все другие команды, `werf render` работает только с уже commited в Git файлами, но поддерживает режим `--dev`._
