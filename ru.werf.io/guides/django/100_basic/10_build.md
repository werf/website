...
2. Запустите его заново: 
    ...
    werf run --docker-options="-d -p 8000:8000 --restart=always" -- runserver 0.0.0.0:8000
    ```
2. Посмотрите, как произойдёт пересборка и запуск, а затем обратитесь к API: http://example.com:8000/test
3. Вы ожидали увидеть сообщение `Our changes`, но **ничего не изменилось**, почему?
