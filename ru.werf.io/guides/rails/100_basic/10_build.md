...
2. Запустите его заново: 
    ...
    werf run --docker-options="--rm -p 3000:3000" -- bundle exec puma
    ```
3. Посмотрите, как произойдёт пересборка и запуск, а затем обратитесь к API: http://example.com:3000/labels
4. Вы ожидали увидеть сообщение `Our changes`, но увидите старый результат. **Ничего не изменилось**, почему?
