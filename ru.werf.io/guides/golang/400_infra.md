...
Мы предполагаем, что к началу прохождения самоучителя вы привязали к своему кластеру какой-то домен.
...
Обычно для каждого стенда (production, staging, test и т.п.) и приложения используется отдельное пространство имён. Таким образом, для приложения `werf-guided-golang` будут созданы пространства имён `werf-guided-golang-production`, `werf-guided-golang-staging` и т.п.
...
- `<namespace>` — название пространства имён в Kubernetes (например, `werf-guided-golang-production`);
...
{% snippetcut name=".helm/templates/deployment.yaml" url="https://github.com/werf/werf-guides/blob/master/examples/gitlab-golang/020-basic-1/.helm/templates/deployment.yaml" %}
{% raw %}
```yaml
    spec:
      imagePullSecrets:
...
В каждом репозитории с кодом, с которым вы будете работать, вам надо будет **прописать в GitLab специальную переменную**. Зайдите в репозиторий и в левой панели нажмите `Settings` -> `CI/CD`. В этом разделе, в главе `Variables`, нужно прописать переменную окружения `KUBECONFIG_BASE64` с нашим `.kube/config` в кодировке base64.
