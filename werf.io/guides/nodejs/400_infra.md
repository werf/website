...
В каждом репозитории с кодом, с которым вы будете работать, вам надо будет прописать в GitLab’е специальную переменную. Зайдите в репозиторий и в левой панели нажмите _Settings -> CI/CD_. В этом разделе, в главе _Variables_, нужно прописать переменную окружения `KUBECONFIG_BASE64` с нашим `.kube/config`, зашифрованным base64.
