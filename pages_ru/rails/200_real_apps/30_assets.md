...
В этой главе рассматриваются особенности организации, сборки и раздачи ассетов (CSS, JS, изображений и других статических файлов). 
...
При разработке обсуживанием всех файлов приложения занимается встроенный RoR-сервер, а компиляция ассетов выполняется на лету во время обработки запроса.
Несмотря на то, что ценные ресурсы Ruby-воркера расходуются неоптимальным образов, такой подход упрощает разработку и привносит ряд преимуществ при интеративной сборке ассетов.
...
Если эти данные не храняться в ассетах, то в таком случае они должны получаться извне.
...
Таким образом, имея доступ к этим переменным окружениям в приложение, можно реализовать специальный API-метод или даже выстроить логику в view-файле. 
...
За подготовку ассетов в нашем приложение будет отвечать webpack.
...
Страница использует стили и JS-скрипт. 
...
Это зарезервированный путь, по которому доступны параметры окружения.
Эти параметры будут храниться в контейнере вместе с NGINX и ассетами, монтироваться из ConfigMap при запуске (подробнее далее).
...
Все изменения в git и можно выполнить выкат приложения [командой `converge`]({{ site.url }}/documentation/reference/cli/werf_converge.html):
...
Список лейблов должен отражать реальное состояние.
