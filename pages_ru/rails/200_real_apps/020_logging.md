...
У задеплоенных в Kubernetes приложений логи всегда должны отправляться на STDOUT/STDERR. Отправка логов на STDOUT/STDERR делает логи доступными для Kubernetes и систем сбора логов, помогает избегать потерь логов при пересоздании контейнеров, а также переполнения логами дисков Kubernetes-нод при сохранении их в файлах внутри контейнеров.

По умолчанию Rails не пишет логи в STDOUT, а сохраняет их в файл. Для того чтобы перенаправить вывод в STDOUT, при использовании стандартной конфигурации, необходимо использовать переменную окружения `RAILS_LOG_TO_STDOUT=1`.

Так как нам не понадобится писать логи куда-либо кроме STDOUT, то, независимо от переменных окружения, логи всегда направляются в STDOUT.
