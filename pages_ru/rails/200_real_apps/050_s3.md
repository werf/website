...
Контейнеры, развертываемые в Kubernetes, часто будут создаваться и удаляться автоматически, вследствие, например, обновления Deployment. Это значит, что мы не можем хранить файлы, создающиеся приложением, прямо в файловой системе контейнера: так файлы будут доступны только одному контейнеру/реплике приложения, а не всем, а также файлы будут удаляться при удалении контейнера. Поэтому контейнеры должны хранить в своей файловой системе только те данные, которые можно потерять.

> _При наличии возможности, файловую систему контейнера даже переключают в read-only, что улучшает безопасность и позволяет убедиться, что приложение действительно не сохраняет данные локально._

Но что делать, если какие-то данные все-таки нужно хранить? Для этого обычно используют развертываемые отдельно базы данных. В частности, для хранения обычных файлов часто используют нереляционные базы данных вроде объектных хранилищ. Особенно часто для хранения файлов используют объектные хранилища, предоставляющие API, совместимый с Amazon S3 API.
...
Базовая конфигурация для Active Storage перенесена в наше приложение из скелета нового Rails-приложения, сгенерированного следующей командой (убрано `--skip-active-storage`):
```shell
...
1. Добавлен package `rails-activestorage` в [package.json]({{ page.base_url | append: page.examples | append: "/package.json" }}).
...
## Добавление endpoints `/upload` и `/download` в приложение

Для демонстрации работы загрузки и отдачи файлов мы добавим два новых endpoints, один из которых будет загружать файл в S3-совместимое объектное хранилище (`/upload`), а другой доставать его оттуда (`/download`).
...
После чего сгенерируем и сохраним новую [db/schema.rb]({{ page.base_url | append: page.examples | append: "/db/schema.rb" }}).

Новые endpoints `/upload` и `/download` добавлены, осталось только настроить для них работу с хранилищем.

## Развертывание и подключение MinIO

Для демонстрации, в качестве S3-совместимого объектного хранилища мы будем использовать MinIO, но вместо него можно использовать и любое другое S3-хранилище, например Amazon S3.

> _При использовании другого хранилища создание нижеописанных StatefulSet и Job для MinIO не требуется._
...
Создадим Job для первичной настройки MinIO:
...
Теперь, если нам требуется работать с файлами, мы можем получать и хранить их в объектном хранилище, а не в файловой системе контейнера. Таким образом Pod'ы приложения могут создаваться и удаляться без проблем: файлы не потеряются, будут доступны с любой реплики Deployment'а приложения, а файловая система на узлах Kubernetes не будет забиваться ненужными файлами. Не забывайте: в контейнере можно хранить только те данные, которые можно потерять. Все остальные данные нужно хранить в соответствующих базах данных/хранилищах.
