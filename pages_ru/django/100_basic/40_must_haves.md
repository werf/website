...
Если бы вы строили процесс деплоя без werf, вручную, то вам бы пришлось задавать строгие правила тегирования образов и соблюдать их (поверьте, это [очень непросто](https://www.youtube.com/watch?v=oh4N2wBJCc8)). В случае с werf пользователю не нужно думать о тегировании и о том, что и как хранится в container registry — он оперирует исключительно именем из werf.yaml (в helm-шаблонах .Values.werf.image.<image_name>, в командах werf run <image_name>), которое никогда не пересекается с тегом. Ниже представлен пример того, как в случае с werf в чартах используется конструкция вида {% raw %}`image: {{ .Values.werf.image.basicapp }}`{% endraw %}:
