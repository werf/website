...
function scrollToAnchor() {
  const $anchor = $('.clickable-header');
  const fixedElementHeight = 120;
  const encodedHash = window.location.hash.substring(1);
  const decodedHash = decodeURIComponent(encodedHash);

  if ($anchor.length > 0) {
    const lastAnchor = $('#configurator-content').find(`#${decodedHash}`)
    $('html, body').stop().animate({
      scrollTop: $(lastAnchor).offset().top - fixedElementHeight
    }, 200);
  }
}
