...
Мы предполагаем, что вы уже [установили werf]({{ site.docsurl }}/installation.html).
...
werf следует принципам гитерменизма — реализует состояние, описанное в git-репозитории. Файлы, не находящиеся в git-репозитории по умолчанию будут игнорироваться. Таким образом, имея исходные коды приложения вы всегда можете реализовать его конкретное работоспособное состояние.
