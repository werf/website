...
Строка `dockerfile: Dockerfile` указывает, что сборочная конфигурация описаны в существующем файле расположенном по пути `Dockerfile`. 
...
_Вы также можете заметить, что `werf run` также осуществляет сборку, т.е. предварительно запускать сборку не обязательно._
...
1. Остановите ранее запущенный werf run (нажав Ctrl+C в консоли, где он запущен) и запустите его заново: 
    ```bash
    $ werf run --docker-options="-d -p 3000:3000 --restart=always" -- node /app/app.js
    ```
...
В описанном сценарии **между шагами 1 и 2 забыли сделать коммит** в git.
...
3. Перезапустить `werf run`
    ...
    werf run --docker-options="-d -p 3000:3000 --restart=always" -- node /app/app.js
    ```
4. Посмотреть на результат браузером
