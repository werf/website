...
Базовый параметр, который может задать пользователь — это **окружение**: `--env ENV`. Это строка, задающая целевую конфигурацию. Данный параметр будет доступен в `werf.yaml` (через конструкцию `{{ .Env }}`) и в шаблонах `.helm/templates` через конструкцию `.Values.werf.env`. Позже, в главе "Работа с инфраструктурой", мы рассмотрим, как значение этой переменной автоматически подставляется CI-системой. Вручную значение можно задать опцией `--env` при запуске `converge`:

```bash
werf converge --repo registry.mydomain.io/werf-guided-project --env production
```

В werf также поддерживаются следующие дополнительные способы конфигурации:

1. Подстановка значений из `values.yaml` и других values-файлов, передаваемых опциями `--values PATH` (по аналогии с Helm).
2. Проброс значений опцией `--set`/`--set-string`/`--set-file` по аналогии с Helm.
3. Подстановка секретных значений из `secret-values.yaml` и других secret-values-файлов, передаваемых опциями `--secret-values PATH`.

Рекомендуется минимизировать количество параметров передаваемых опциями командной строки (лучше всего передавать ). Лучше всего явно через параметр указывать **окружение**: `--env ENV`. Все остальные параметры в данном случае должны зависеть только от переданного окружения. Такая настройка гарантирует простоту _воспроизводимости конфигурации_ на разных машинах. Окружения обычно заданы статически и выбирая конкретное окружение пользователь получает весь необходимый набор параметров для запуска приложения в данном окружении. Исходя из таких соображений нужно правильно организовывать values и secret-values файлы (см. далее) и по-возможности избегать использования опций `--set`/`--set-string`/`--set-file`/`--values`/`--secret-values`.

<!-- NOTE: Возможно в werf появится возможность автоматически подхватывать values-файлы для конкретного окружения в зависимости от переданного параметра --env: .helm/values.ENV.yaml, .helm/secret-values.ENV.yaml. Пока это только идея, будем обсуждать. -->
