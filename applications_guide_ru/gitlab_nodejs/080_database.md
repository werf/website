...
Пароль от базы данных мы тоже конфигурируем, но хранить их нужно в секретных переменных. Для этого стоит использовать механизм секретных переменных. *Вопрос работы с секретными переменными рассматривался подробнее, [когда мы делали базовое приложение](020_basic/20_iac.html#secret-values-yaml)*.
...
Для того, чтобы PersistentVolume корректно создал хранилище — нам нужно правильно указать, **на каком узле** и **в какой папке** будет наш localstorage.

В аттрибуте `nodeAffinity` указываем нужный узел (в примере ниже этот узел называется `article-kube-node-2`, посмотреть весь список узлов можно через `kubectl get nodes`) 
...
На указанном узле нужно **вручную создать папку** и прописать её в конфиге PersistentVolume:
...
{% offtopic title="Но каким образом под с нашей базой поймет что ему нужно использовать именно этот PersistentVolume?" %}
...
PV как будто "зависнет" в процессе удаления. Это корректное поведение, благодаря `pv-protection`.
...
То увидите аттрибут `finalizers`:
...
Они защищают ваши данные от случайного удаления. Если же вы настаиваете — надо отредактировать pv, удалив из yaml-а конфигурации описанные строки с `pv-protection`.
